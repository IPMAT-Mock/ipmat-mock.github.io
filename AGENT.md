...
| Admin BE | `app/server.py` (:5057, 127.0.0.1) | `/api/bank|blueprints|assemble|papers/publish|usage|config|students/*|llm/*`; `/api/bank` filters `section|topic|subtopic|difficulty|q`; bank add rejects unknown subtopics with the bucket's valid list; paper breakdown (`GET /api/papers/<id>/breakdown`: difficulty/section/topic/subtopic splits, answer balance, integrity, blueprint expected-vs-actual) + unpublish (manifest only) + delete (manifest + both files); LLM loop gated by `app/llm.config.json` (`enabled`); generate = 1 batch prompt, max 2 provider calls, visible progress + elapsed timer, double-click lock, call count + cooldown + manual Retry on 429/503 (Retry-After honored); prefer `gemini-2.5-flash`; publish rejects duplicate paper_id with 409 unless `overwrite:true`; `/api/usage` maps qid → manifest papers (+orphan paper files missing from the manifest); student signup (`POST /api/students/signup`, unique email 409, scrypt-hashed passwords in git-ignored `data/students.json`) + signin (`POST /api/students/signin`, generic 401) + admin list/delete/reset-password |
