...
| Admin UI | `app/admin.html` | dashboard, bank browser+add (Used-in column + used/never-used filter, ⚠ flags paper files missing from the manifest; Subtopic column + filter, canonical tags enforced on add), blueprint manager (CRUD + dry-run vs bank), paper wizard (kind/count/subject/single-or-mix/difficulty, unique auto-suggested Paper ID, overwrite confirm, saved-file message), papers manager (per-paper breakdown, blueprint match, unpublish/delete), students manager (search/reset-pw/delete), LLM, config |
